...
**Explain other ways of structuring shiny app code**
**Sort out sizing of built-in apps/put border around them**
**Check how all apps display**
...
- The above code consists of two functions for performing the two separate roles that were described above, and then a third function which actually tells R to run the app:
    ...
    + **shinyApp** is the function which tells R to run the other two components as a `shiny` app.
    
- Although all `shiny` apps need to have these two components in order to work, an app can actually be structured in two different ways:
    1) Through a single call to `shiny::shinyApp(ui = [UI Code], server = [Server Code])` (all of the examples included in this document use this form as it's the easiest way of embedding apps within R Markdown code chunks to create interactive documents);
    2) By defining two separate `ui` and `server` objects outside of the call to `shiny::shinyApp` and then passing them to the latter function (this also allows you to keep the two parts in two separate R scripts, or separate R chunks within an interactive R Markdown document).
...
![Fig.1 Input functions in base shiny](`r here('images', 'shiny_input_function_examples.png')`)
...
![Fig.2 Output functions in base shiny](`r here('images', 'shiny_output_function_examples.png')`)
...
![Fig.3 Render functions in base shiny](`r here('images', 'shiny_render_function_examples.png')`)
...
```{html raw_html_ui_example, eval=FALSE}
...
shinyApp(ui = fluidPage(fluidRow(column(4, offset = 8, plotOutput('hist')))), server = function(input, output) {})
```
...
![Fig.4 Raw HTML vs HTML + CSS](`r here('images', 'html_plus_css_webpage_example.png')`)
